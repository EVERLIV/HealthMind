    ...
    try {
      ...
      try {
        ...
        if (healthProfile?.profileData) {
          const profileData = healthProfile.profileData as any;
          userContext = `👤 Ваш профиль: Возраст: ${profileData?.age || 'не указан'}, Пол: ${profileData?.gender || 'не указан'}`;
        }
        
        if (openaiError.status === 429) {
          imageAnalysis = `📸 **EVERLIV Помощник - Анализ изображения (офлайн режим)**

🤖 **Система ИИ временно недоступна, но я готов помочь!**

**📋 Для точного анализа, пожалуйста, опишите:**
• Что именно на фотографии?
• Где расположена проблема?
• Когда впервые заметили?
• Есть ли симптомы (зуд, боль, жжение)?
• Размер, цвет, форма образования?
• Принимаете ли лекарства?
• Были ли подобные проблемы раньше?

**🩺 Общие медицинские рекомендации:**
• При кожных проблемах - не расчесывать область
• Соблюдать гигиену без агрессивных средств  
• При воспалении - холодные компрессы
• Избегать новой косметики и аллергенов
• Делать фото динамики изменений

**🔬 Для анализов крови:**
• Опишите показатели и их значения
• Укажите референсные нормы из бланка
• Расскажите о симптомах или жалобах

${userContext}

⚠️ **ВАЖНО**: При серьезных изменениях, боли или ухудшении - обязательно обратитесь к врачу для очного осмотра!

💬 **Опишите проблему подробнее, и я дам персональные рекомендации!**`;
        } else {
          imageAnalysis = `🔧 **Технические неполадки - но я помогу!**

**💭 Опишите что на изображении:**
• Кожная проблема, симптомы, родинка?
• Результаты медицинских анализов?
• Медицинская документация?

**🎯 После описания я дам:**
• Персональные рекомендации
• Объяснение медицинских терминов
• Советы по уходу и лечению
• Информацию когда обратиться к врачу
